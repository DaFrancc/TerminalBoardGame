...
use crate::message::{
    receive_message_with_timeout, serialize_message, ClientMessage, InternalMessage,
    ServerStartMessage, WaitTimeSec,
};
...
async fn serialize_players<T: Clone>(
    players_list: &PlayersList<T>,
    include_data: bool,
) -> Vec<PlayerSerialize<T>> {
    let players = players_list.read().await;

    players
        .values()
        .map(|player| PlayerSerialize {
...
async fn client_join<T: Clone + Serialize + for<'a> Deserialize<'a>, U: Serialize + Clone>(
    ...
    players_list: &mut PlayersList<T>,
    num_players: &AtomicU8,
) -> Result<usize, TBGError> {

    // Wait for the client to send join message
    let message: ServerStartMessage<T> =
        receive_message_with_timeout(&mut stream, Duration::from_secs(WaitTimeSec::Med as u64))
            .await?;
    ...
    match message {
        ServerStartMessage::Join(name) => {
            ...
            // Add player, if name already exists, reject client
            let player_id = match players_list.add_player(name.clone(), stream, None).await {
                ...
                Err((mut s, e)) => {
                    ...
                    if let Err(e) = s.flush().await {
                        eprintln!("Failed to flush client to disk: {}", e);
                    }
            ...
            let p_serialized = serialize_players(players_list, false).await;

            players_list
                .write()
                .await
                .get_mut(&player_id)
                .ok_or(TBGError::InvalidID)?
                .stream
                .write_all(
                    serialize_message(&ClientMessage::<T, U>::SendAll(p_serialized))?.as_slice(),
                )
                .await;
            println!("{name} joined");
        ...
        _ => Err(TBGError::FailToConnect(match stream.peer_addr() {
            Ok(x) => x.to_string(),
            Err(_) => String::from("BAD IP"),
        })),
    }
}
