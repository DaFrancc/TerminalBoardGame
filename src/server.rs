...
use crate::message::{receive_message_with_timeout, serialize_message, ClientMessage, InternalMessage, ServerStartMessage};
    ...
    -> Result<usize, TBGError>
{
    ...
    match message {
        ServerStartMessage::Join(name) => {
            ...
            player.lock().await.stream.write_all(serialize_message(&ClientMessage::<Option<T>, U>::SendAll(serialize_players(players_list.clone(), false).await)).as_slice()).await?;
            println!("{name} joined");
...
) -> Result<(), TBGError>
{
    ...
    match client_in_lobby(player_id, players.clone(), num_players.clone(), num_votes.clone(), broadcast_rx, mpsc_tx).await {
        Ok(_) => {},
        Err(e) => return Err(e)
    }
...
async fn client_in_lobby_default<T: Clone + Serialize + for<'a> Deserialize<'a>, U: Serialize + Clone + for <'a> Deserialize<'a>>(
    player_id: usize,
    players_list: Arc<PlayersList<T>>,
    num_players: Arc<AtomicU8>,
    num_votes: Arc<AtomicU8>,
    ...
    mpsc_tx: Sender<InternalMessage<U>>) -> Result<(), TBGError>
{
    ...
    'lobby_loop: loop {
        if let Ok(msg) = broadcast_rx.try_recv() {
            ...
            match serialized {
                Ok(s) => {
                    ...
                    p_lock.stream.write_all(serialize_message(&s).as_slice()).await.unwrap_or_else(|e| eprintln!("Failed to send message to player: {}", e));
        ...
        let message = match receive_message_with_timeout::<ServerStartMessage<U>>(
            &mut p_lock.stream, Duration::from_millis(20)).await
        {
            Some(m) => m,
            None => continue,
        };
        match message {
            ServerStartMessage::VoteStart(vote) => {
                ...
                if let Err(_) = mpsc_tx.send(InternalMessage::VoteStart(p_lock.name.clone(), vote)).await {
            ...
            ServerStartMessage::Exit => {
                if let Err(_) = mpsc_tx.send(InternalMessage::PlayerLeft(p_lock.name.clone())).await {
                    eprintln!("Failed to send message to server");
                }
...
pub async fn server_thread<T: Clone + Serialize + for<'a> Deserialize<'a> + Send + Sync + 'static,
